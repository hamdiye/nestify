...
public class EventPolicy {
	...
	/**
	 * Validates that the user is a member of at least one house before creating an event.
	 *
	 * @param userId the ID of the acting user
	 * @throws RuntimeException if the user is not a member of any house
	 */
	public void validateEventCreation(Long userId) {
		if (!userServiceHelper.isUserMemberOfAnyHouse(userId)) {
			throw new RuntimeException("Etkinlik oluşturmak için en az bir eve üye olmanız gerekiyor.");
		}
	}
}
